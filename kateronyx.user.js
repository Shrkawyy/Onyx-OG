...
(function () {
  ...
  function isOfficialRuntimeNode(el) {
    ...
    if (tag === 'CANVAS' && el.id !== 'gameCanvas' && el.id !== 'canvas' && el.id !== 'minimap-nodes' && el.id !== 'onyx-ffa-minimap-skins' && el.id !== 'onyx-ffa-world-skins') return true;
    return false;
  }
  ...
  async function mount(baseUrl) {
    ...
    var skinsJs = await gmGet(baseUrl + 'onyx-ffa-skins.js?v=' + v);
    ...
    await injectScript(skinsJs, 'onyx-ffa-skins.js');
    await injectScript(
